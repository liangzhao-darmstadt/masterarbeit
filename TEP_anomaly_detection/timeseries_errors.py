# Modified by LZ
# based on https://github.com/signals-dev/Orion/blob/master/orion/primitives/timeseries_errors.py
...
def _area_error(y, y_hat, score_window=10):
    ...
    # added by LZ
    numOfMultivariant = y.shape[1]
    smooth_y_list = []
    smooth_y_hat_list = []

    # modified by LZ
    for m in range(numOfMultivariant):
        smooth_y_list.append(pd.Series(y[:, m]).rolling(
            score_window, center=True, min_periods=score_window // 2).apply(integrate.trapz))
        smooth_y_hat_list.append(pd.Series(y_hat[:, m]).rolling(
            score_window, center=True, min_periods=score_window // 2).apply(integrate.trapz))

    smooth_y = np.asarray(smooth_y_list).T
    smooth_y_hat = np.asarray(smooth_y_hat_list).T
    print("smooth_y_hat.shape", smooth_y_hat.shape)
...
def reconstruction_errors(y, y_hat, step_size=1, score_window=10, smoothing_window=0.01,
    ...
    for i in range(num_errors):
        ...
        if intermediate:
            ...
            # added by LZ
            predictions_vs.append([[
                np.min(np.asarray(intermediate), axis=0),
                np.percentile(np.asarray(intermediate), 25, axis=0),
                np.percentile(np.asarray(intermediate), 50, axis=0),
                np.percentile(np.asarray(intermediate), 75, axis=0),
                np.max(np.asarray(intermediate), axis=0)
            ]])
    ...
    print("predictions_vs.shape: ", predictions_vs.shape)
    ...
    if smooth:
        for m in range(errors.shape[1]):
            errors_list.append(pd.Series(errors[:, m]).rolling(smoothing_window, center=True,
